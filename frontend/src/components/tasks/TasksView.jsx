...
import { 
  ...
  Calendar, CheckCircle, ShieldAlert, ArrowUpDown, ChevronRight, User as UserIcon
} from 'lucide-react';
...
export const TasksView = () => {
  ...
  return (
    <div id="tasks-view" className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition"
          >
      ...
      {/* Filters Toolbar */}
      <div className="flex flex-wrap items-center gap-2.5 p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs">
        {/* Search */}
        <div className="relative min-w-[200px] max-w-xs flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1 text-xs rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-hidden"
          />
        </div>
        ...
        <select
          ...
          className="px-2.5 py-1 text-xs rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-hidden"
        >
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <select
            ...
            className="px-2 py-1 text-xs rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-hidden"
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xs">
        {filteredTasks.length === 0 ? (
          <div className="py-16 text-center text-slate-400 p-8">
            <CheckSquare className="w-12 h-12 mx-auto mb-3 text-slate-300 dark:text-slate-700" />
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">No tasks match your criteria</p>
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition ${
                        isUnderReview ? 'bg-purple-50/30 dark:bg-purple-950/10' : ''
                      }`}
                    >
                      ...
                      <td className="p-3.5 max-w-xs">
                        <div className="font-semibold text-slate-900 dark:text-slate-100 line-clamp-1 mb-0.5">
                          {t.title}
                        </div>
                        {(t.labels || []).length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {(t.labels || []).slice(0, 3).map(l => (
                              <span key={l} className="text-[9px] px-1.5 py-0.2 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 font-medium">
                                {l}
                              </span>
                            ))}
                          </div>
                      ...
                      <td className="p-3.5 text-slate-500">{project?.code || 'PRJ'}</td>
                      <td className="p-3.5">
                        {assignee ? (
                          <div className="flex items-center gap-2">
